...
class TeamAssigner:
    def __init__(self, color_threshold: float = 100.0, kmeans_n_init: int = 10) -> None:
        ...
        self._n_init = kmeans_n_init

    # --- colour sampling ---------------------------------------------------
    def _player_color(self, frame: FrameBGR, box: BBox) -> np.ndarray:
        ...
        if patch.size == 0 or patch.shape[0] < 5 or patch.shape[1] < 5:
            return np.zeros(3)
        km = KMeans(n_clusters=2, init="k-means++", n_init=self._n_init, random_state=42)
    ...
    def assign_team_colors(self, frame: FrameBGR, players: dict[str, BBox]) -> None:
        ...
        km = KMeans(n_clusters=4, init="k-means++", n_init=self._n_init, random_state=42)
        ...
        self._kmeans = km
        for pid, label, color in zip(ids, labels, colors, strict=True):
            if label in top:
    ...
    def team_of(self, frame: FrameBGR, box: BBox, track_id: str) -> int | None:
        ...
        best_dist = float("inf")
        for team in (1, 2):
            d = float(np.linalg.norm(color - self.team_colors[team]))
            if d < best_dist:
                best_dist, best_team = d, team
            for libero in self.libero_colors.get(team, []):
                dl = float(np.linalg.norm(color - libero))
