...
Что настраивается (все переключатели — в конфиге, реализации подменяются в
composition root, пайплайн не меняется):

| Настройка | Что делает |
|---|---|
| `video.prefetch_queue` | декодирование в фоновом потоке — перекрывается с инференсом вместо того, чтобы блокировать его (`PrefetchVideoSource`) |
| `people.detect_interval` | полнокадровый инференс людей раз в N кадров, между ними переиспользуется прошлый результат (`IntervalPeopleDetector`) |
| `detector.mode` | `motion_roi` (CPU motion-mask → YOLO-классификатор по ROI) против `direct` (один YOLO-детект по кадру, без CPU-препроцесса) |

Всегда включено: `torch.inference_mode()`, fp16 на CUDA, прогрев модели
(первый кадр больше не платит за автотюнинг ядер), ленивая аллокация overlay
(headless-прогон не копирует кадр впустую), альфа-смешение миникарт только по
ROI (**79× быстрее** полнокадрового варианта, результат байт-в-байт тот же —
см. `tests/test_blit.py`).

`detector.mode` и `people.detect_interval` — компромиссы «скорость против
точности»: замеряйте на своём материале. Остальное выигрыш без компромисса.
